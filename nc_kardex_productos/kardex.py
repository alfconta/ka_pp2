...
class kardex_productos_inventario(models.TransientModel):
    ...
    def _action_imprimir_grupo_inventario_excel(self, ):
        ...
        worksheet.write(0, 0, _('Kardex report product for Group'), column_heading_style)
...
where pt.categ_id = %s
    """
            ...
            for prod in g:
                ...
                query_saldo_anterior_param = (producto,ubicacion,compan,producto,ubicacion,compan,date_from)
                ...
                query_movimiento_param = (producto,ubicacion,compan,producto,ubicacion,compan, date_from, date_to)
                ...
                for mov in movim: 
                    ...
                    inventario= self._buscar_factura_reportexcel(movimiento)
                    worksheet.write(tm + ini2+ conteo, 14, inventario)
...
""" + query_total + """
    ...
    """ + query_total + """
        ...
        query_saldo_final_param = (producto,ubicacion,compan, producto,ubicacion,compan, date_to)
...
---------------------------------------- 4) EMPIEZA LA UNION
                            ...
                            from 
                            (
                                select  pt.name as name_product,pp.default_code,ru.id as user_id,rp.name as user_name,sl.id as location_id,sl.complete_name as locat_name,sm.id,sm.create_date   AT TIME ZONE 'UTC' AT TIME ZONE '"""+zh+"""' as date_expected,sm.product_id,sm.name,sm.company_id,sm.product_qty as u_entrada,(sm.product_qty * 0)u_salida,
                            ...
                            ------------- 1) unimos salidas
                            select name_product,default_code,user_id,user_name,location_id,locat_name,id,date_expected ,product_id,name,company_id, u_entrada,u_salida,costo_unit
                            ,v_entrada,v_salida,v_saldo,state,origin,reference,
                            usage,complete_name,ubicacion,create_uid,inventory_id,picking_id  
                            from
                            (
                                
                                    select pt.name as name_product,pp.default_code,ru.id as user_id,rp.name as user_name,sl.id as location_id,sl.complete_name as locat_name,sm.id,sm.create_date  AT TIME ZONE 'UTC' AT TIME ZONE '"""+zh+"""' as date_expected,sm.product_id,sm.name,sm.company_id,(sm.product_qty * 0) as u_entrada,
